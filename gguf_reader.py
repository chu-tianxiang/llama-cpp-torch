# Modified from https://github.com/ggerganov/llama.cpp/blob/master/gguf-py/gguf/gguf_reader.py
#
...
class GGUFReader:
    ...
    def _build_tensors(self, start_offs: int, fields: list[ReaderField]) -> None:
        ...
        for field in fields:
            ...
            #elif ggml_type == GGMLQuantizationType.Q4_0:
            #    item_type = np.dtype([('d', np.half), ('qs', np.uint8, (16,))])
            #elif ggml_type == GGMLQuantizationType.Q4_1:
            #    item_type = np.dtype([('d', np.half), ('m', np.half), ('qs', np.uint8, (16,))])
            #elif ggml_type == GGMLQuantizationType.Q5_0:
            #    item_type = np.dtype([('d', np.half), ('qh', np.uint8, (4, )), ('qs', np.uint8, (16,))])
            #elif ggml_type == GGMLQuantizationType.Q5_1:
            #    item_type = np.dtype([('d', np.half), ('m', np.half), ('qh', np.uint8, (4, )), ('qs', np.uint8, (16,))])
            #elif ggml_type == GGMLQuantizationType.Q8_0:
            #    item_type = np.dtype([('d', np.half), ('qs', np.uint8, (32,))])
            #elif ggml_type == GGMLQuantizationType.Q8_1:
            #    item_type = np.dtype([('d', np.float), ('s', np.float), ('qs', np.uint8, (32,))])
            #elif ggml_type == GGMLQuantizationType.Q2_K:
            #    item_type = np.dtype([('scales', np.uint8, (QK_K // 16,)), ('qs', np.uint8, (QK_K // 4,)), ('d', np.half), ('dmin', np.half)])
            #elif ggml_type == GGMLQuantizationType.Q3_K:
            #    item_type = np.dtype([('hmask', np.uint8, (QK_K // 8,)), ('qs', np.uint8, (QK_K // 4,)), ('scales', np.uint8, (12,)), ('d', np.half)])
            #elif ggml_type == GGMLQuantizationType.Q4_K:
            #    item_type = np.dtype([('d', np.half), ('dmin', np.half), ('scales', np.uint8, (12,)), ('qs', np.uint8, (QK_K // 2,))])
            #elif ggml_type == GGMLQuantizationType.Q5_K:
            #    item_type = np.dtype([('d', np.half), ('dmin', np.half), ('scales', np.uint8, (12,)), ('qh', np.uint8, (QK_K // 8,)), ('qs', np.uint8, (QK_K // 2,))])
            #elif ggml_type == GGMLQuantizationType.Q6_K:
            #    item_type = np.dtype([('ql', np.uint8, (QK_K // 2,)), ('qh', np.uint8, (QK_K // 4,)), ('scales', np.int8, (QK_K // 16,)), ('d', np.half)])
            #elif ggml_type == GGMLQuantizationType.Q8_K:
            #    item_type = np.dtype([('d', np.half), ('qs', np.int8, (QK_K,)), ('bsums', np.int16, (QK_K // 16,))])
            else:
                item_type = np.uint8
                item_count = n_bytes
...
def convert_to_state_dict(checkpoint, save_dir):
    ...
    rope_base = float(result.fields['llama.rope.freq_base'].parts[-1])
